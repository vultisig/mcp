...
// SatsPerByte returns the recommended fee rate for a chain (e.g. "DASH", "ZEC").
// Uses MayaChain inbound_addresses GasRate.
func (c *Client) SatsPerByte(ctx context.Context, chain string) (uint64, error) {
	...
	for _, addr := range addresses {
		if addr.Chain == chain {
			if addr.Halted {
				return 0, fmt.Errorf("chain %s is currently halted on MayaChain", chain)
			}
...
func (c *Client) getInboundAddresses(ctx context.Context) ([]inboundAddress, error) {
	url := c.baseURL + "/mayachain/inbound_addresses"
	...
	if err != nil {
		return nil, fmt.Errorf("mayachain inbound_addresses: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("mayachain inbound_addresses returned %d", resp.StatusCode)
	}

	var addresses []inboundAddress
	err = json.NewDecoder(resp.Body).Decode(&addresses)
	if err != nil {
		return nil, fmt.Errorf("mayachain: decode inbound_addresses: %w", err)
	}
