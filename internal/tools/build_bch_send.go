...
func newBuildBCHSendTool() mcp.Tool {
	return mcp.NewTool("build_bch_send",
		mcp.WithDescription(
			"Build an unsigned Bitcoin Cash PSBT for a send or swap. "+
		...
		mcp.WithString("to_address",
			mcp.Description("Recipient Bitcoin Cash address (CashAddr or legacy format)"),
			mcp.Required(),
		),
...
func handleBuildBCHSend(store *vault.Store, bcClient *blockchair.Client) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		senderAddr, derivedPubKey, _, err := address.GetAddress(v.ECDSAPublicKey, v.ChainCode, common.BitcoinCash)
		if err != nil {
		...
		dashboard, err := bcClient.GetAddressDashboard(ctx, "Bitcoin-Cash", senderAddr)
		if err != nil {
		...
		recipientScript, err := bchChain.addressToPkScript(toAddress)
		if err != nil {
			return mcp.NewToolResultError(fmt.Sprintf("invalid to_address: %v", err)), nil
		}

		changeScript, err := bchChain.addressToPkScript(senderAddr)
		if err != nil {
		...
		err = btcsdk.PopulatePSBTMetadata(result, bcClient.ChainFetcherWithCtx(ctx, "Bitcoin-Cash"))
		if err != nil {
