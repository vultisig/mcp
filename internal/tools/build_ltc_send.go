...
func newBuildLTCSendTool() mcp.Tool {
	return mcp.NewTool("build_ltc_send",
		mcp.WithDescription(
			"Build an unsigned Litecoin PSBT for a send or swap. "+
		...
		mcp.WithString("to_address",
			mcp.Description("Recipient Litecoin address"),
			mcp.Required(),
		),
		...
		mcp.WithNumber("fee_rate",
			mcp.Description("Fee rate in sat/vB (use ltc_fee_rate tool to get recommended rate)"),
			mcp.Required(),
		),
		mcp.WithString("memo",
...
func handleBuildLTCSend(store *vault.Store, bcClient *blockchair.Client) server.ToolHandlerFunc {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		senderAddr, derivedPubKey, _, err := address.GetAddress(v.ECDSAPublicKey, v.ChainCode, common.Litecoin)
		if err != nil {
		...
		dashboard, err := bcClient.GetAddressDashboard(ctx, "Litecoin", senderAddr)
		if err != nil {
		...
		recipientScript, err := ltcChain.addressToPkScript(toAddress)
		if err != nil {
			return mcp.NewToolResultError(fmt.Sprintf("invalid to_address: %v", err)), nil
		}

		changeScript, err := ltcChain.addressToPkScript(senderAddr)
		if err != nil {
		...
		changeIdx := 1
		utxoBuilder := btcsdk.NewBuilder(5460)
		...
		err = btcsdk.PopulatePSBTMetadata(result, bcClient.ChainFetcherWithCtx(ctx, "Litecoin"))
		if err != nil {
