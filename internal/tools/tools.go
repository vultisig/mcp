...
func RegisterAll(s *server.MCPServer, store *vault.Store, pool *evmclient.Pool, cgClient *coingecko.Client, bcClient *blockchair.Client, swapSvc *swap.Service, tcClient *thorchain.Client, mcClient *mayachain.Client, solClient *solanaclient.Client, jupClient *jupiter.Client, xrpClient *xrpclient.Client, esClient *etherscan.Client) error {
	...
	s.AddTool(newGetContractABITool(), handleGetContractABI(esClient))
	s.AddTool(newGetTxHistoryTool(), handleGetTxHistory(store, esClient))
